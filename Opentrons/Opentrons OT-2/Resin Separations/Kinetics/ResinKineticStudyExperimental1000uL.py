...
-1 set of 50 mL Falcon tubes in a 6-tube rack : Location 1
-2 sets of 15ml Falcon tubes in a 15-tube rack : Location 2, 3
-1000 uL tips in a 96-tip rack : Location 4
...
metadata = {
    'apiLevel': '2.18',
    'protocolName': 'ResinKineticStudy1000uL_OpentronsOT2',
    'author': "Brison Cowan",
...
def add_parameters(parameters: protocol_api.Parameters):
    ...
    parameters.add_int(variable_name='number_of_source_tubes', display_name='Number of source tubes',
                       description='Number of source tubes for the experiment', default=1, minimum=1, maximum=2)
    ...
    parameters.add_int(variable_name='number_samples', display_name='Number of samples to collect',
                       description='Number of samples to collect (excluding t_0)', default=4, minimum=0, maximum=9)
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    for i in range(number_of_source_tubes):
        ...
        take_sample_and_mix(pipette, source_tube_list[i], tubes_list[i],
    ...
    for x in range(number_samples):
        ...
        for j in range(number_of_source_tubes):
            take_sample_and_mix(pipette, source_tube_list[j], tubes_list[3 * x + j + 3], lowest_depth, middle_depth,
...
def take_sample_and_mix(pipette_name, source_tube, tube, lowest_depth, middle_depth, transfer_volume,
    ...
    if skip_mix == False:
        ...
        pipette_name.blow_out(source_tube.top()) # blow out any remaining liquid
